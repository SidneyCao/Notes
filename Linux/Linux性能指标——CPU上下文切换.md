...
以下几种情况会触发进程上下文切换：  
- 1. CPU时间片结束。CPU时间会被分为一段段的时间片，再被分配给进程，当进程运行的这个时间片耗尽时，会由系统切换到别的进程。
- 2. 系统资源不足（比如内存，I/O）时。系统会将进程挂起，等到资源满足需求时再重新运行。
- 3. 进程主动挂起。 比如运行了sleep函数。
- 4. 存在优先级更高的进程时。 当有优先级更高的进程时，当前进程会被挂起，运行优先级高的进程。
- 5. 发生硬件中断。 此时进程会被挂起，转而运行内核中的硬件中断服务。  
<br>
